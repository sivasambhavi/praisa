...
def test_can_retrieve_patient_with_bad_mobile():
    ...
    with get_db() as db:
        query = text("SELECT patient_id FROM patients WHERE length(mobile) < 10 LIMIT 1")
...
def test_typo_matching_resilience():
    ...
    if row:
        ...
        print(f"Successfully matched 'Ramesh' to '{db_patient['name']}' with score {result['match_score']}")
...
def test_invalid_abha_handling():
    ...
    with get_db() as db:
        query = text("SELECT abha_number FROM patients WHERE length(abha_number) != 14 LIMIT 1")
        row = db.execute(query).first()
